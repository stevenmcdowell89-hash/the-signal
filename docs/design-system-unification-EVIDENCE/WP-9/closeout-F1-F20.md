...
| **F-5** | ~444KB full-bundle ships deleted-format CSS | Per-format manifest bundling; budgets hard-fail in stitcher | editorial **107.6KB**/120 cap, event **101.8KB**/160 (`references/css-manifests/*.txt`, stitcher byte print). NB: both grew ~5KB after WP-5 expanded `core/14-motion.css` (WP-3 measured 102/96KB); still under budget, gate passes (WP-9 audit finding #1) |
