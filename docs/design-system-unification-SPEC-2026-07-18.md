...
4. **The weekly is protected.** Transmission was redesigned this month, is deterministic, and golden-verified. `assets/css/weekly/00-transmission.css` visual output must not change. `scripts/verify-weekly-golden.sh` must pass after every phase. Any weekly-visible diff is a phase failure.
