...
**Critical repair-brief rule:** if a chapter fails the teaching-density gate, the repair brief MUST say explicitly: *do NOT fix this by adding encyclopedic prose, lit-review paragraphs, or thesis statements. Fix it by giving the actor in the existing scene the institutional action the chapter exists to explain — let the mechanism happen on the page, not next to it.* This protects against the obvious bad repair (which would re-introduce the Yellow Turban failure mode). After repair, BOTH gates re-run on a fresh paragraph sample — the narrative-voice gate confirms the chapter hasn't drifted back into academic register; the teaching-density gate confirms the new prose actually conveys the mechanism.

**One document-level check** also runs at Phase 7: list every chapter's opening sentence. If more than five chapters open with the date-place-person vignette template ("On [date] in [place], [person] [did concrete thing]"), the issue fails for **formula mannerism**. Repair: rewrite the openings of the over-budget chapters, varying narrative entry — primary-source quotation that opens in a voice; a single number with the mechanism behind it; a specific document at a specific desk; a counterfactual. NOT theses ("This chapter will explain…") — that's the Yellow Turban move. Always narrative; just a different angle of entry.

This is the only Phase 7 gate-pair that doesn't have a script — by design. The point is to refuse to ship prose that reads as a paper OR prose that reads as well-decorated but content-empty magazine essay. A regex can't measure either, and a writer pushed too hard against one failure mode will land in the other.
