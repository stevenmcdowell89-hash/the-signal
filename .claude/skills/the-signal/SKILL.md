...
**Deep Dive only — teaching-density spot check (v8.22.12, runs alongside the narrative-voice gate).** The narrative-voice check above catches the Yellow Turban failure mode (academic essay register, lit-review walls, throat-clearing). It does NOT catch the WWI Deep Dive failure mode (magazine-essayist register that performs comprehension without conveying it — formulaic vignette openings, contrastive pivots, parallel triplets, withheld-word endings, characteristic-listing in place of mechanism, "no X / no Y / no Z" domino lists of recognisable nouns). A paragraph can pass the Sunday-morning narrative check and still teach the reader nothing.

The two gates run in series and are NOT in tension. The right answer is **scenes that carry mechanism** — the actor IS the institution doing the institutional thing. The Compromise of 1867 is Franz Joseph at his second coronation in Buda, not a pause in the narrative to explain the constitutional structure of the Dual Monarchy. Both gates pass on the same paragraph when the scene teaches the mechanism through its action.
...
> "After this paragraph, can the reader explain to a friend something concrete about how the institution, system, or event actually worked? Not just an image of it, not just a characteristic — an actual mechanism they could re-state in their own words?"

The pass bar is **re-explainable understanding**, not memorable atmosphere. A paragraph that gives the reader a vivid image of Franz Joseph's chair-brake but nothing about how the Dual Monarchy actually distributed authority between Vienna and Budapest fails. A paragraph that shows Franz Joseph travelling to Buda for his second coronation, swearing his Hungarian oath, and ten years later breaking the tie on the customs treaty renewal passes both gates — it has scene, character, and mechanism in the same prose.

**Repair-brief rule — two acceptable fixes, one rejected one.** If a chapter fails the teaching-density gate, the repair brief offers the writer a choice:
1. **Best:** give the actor in the existing scene the institutional action — let the mechanism happen on the page through what someone does.
2. **Acceptable fallback:** replace the substance-free paragraph with direct informational prose, Wikipedia-style — substance stated plainly, no rhetorical pose. Honest restraint.

**NOT acceptable:** rewriting the paragraph in another magazine-essayist register (different vignette, different rhetorical pose). That just changes costumes and keeps performing without substance. Both Yellow Turban (academic-essayist costume) and WWI (magazine-essayist costume) failed the same way — performance without substance. Wikipedia-style direct prose wears no costume; it's the safe landing when scene-with-mechanism doesn't fit.

After repair, BOTH gates re-run on a fresh paragraph sample. Narrative-voice gate catches drift into Yellow Turban academic register. Teaching-density gate catches drift back into performed-empty magazine register. Direct informational prose passes the teaching-density gate trivially (it's all substance) and passes the narrative-voice gate as long as it's not throat-clearing or scholar-impressing — just plain explanation reads as honest, not academic.

**One document-level check** also runs at Phase 7: list every chapter's opening sentence. If more than five chapters open with the date-place-person vignette template ("On [date] in [place], [person] [did concrete thing]"), the issue fails for **formula mannerism**. Repair options: (a) vary the narrative entry — primary-source quotation, a single number with the mechanism behind it, a specific document at a specific desk, a counterfactual; or (b) open with a direct declarative framing sentence like a Wikipedia section opener — honest restraint, not throat-clearing. Bare lecturer's throat-clearing ("It is worth, before the long-shadow chapter, going back…") is rejected — that's mid-pose between narrative and direct prose, the worst of both.

This is the only Phase 7 gate-pair that doesn't have a script — by design. The point is to refuse to ship prose that performs without substance, in either register. A regex can't measure that; both gates are honor-system at the LLM layer.
