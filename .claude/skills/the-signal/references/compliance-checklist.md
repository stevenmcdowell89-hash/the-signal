...
- [ ] **HARD CHECK -- No result/fixture that hasn't happened yet (v8.28).** Any stated sports result, score, qualifying outcome, or league standing must have ACTUALLY OCCURRED by the issue date and trace to a source reporting it as happened — not a preview, not a prior-year result pulled forward. The Monaco "Norris on pole" failure (qualifying a week away, asserted as fact) is this class. If the event is in the future relative to the issue date, it is written as upcoming, never as a result. And every load-bearing fact must trace to the research bundle (RT-22) — the prose adds no facts the research didn't supply.
