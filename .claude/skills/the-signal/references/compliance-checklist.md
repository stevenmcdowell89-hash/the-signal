...
- [ ] **No "X, Y, and Z" rule-of-three list-padding.** When the agent writes "two consecutive Chinese champions, the strongest cohort of Chinese players ever in the qualifying draw, and the Crucible's 50-year hosting deal expires next year", check whether the middle item is genuinely independent or is just rephrasing the first. If it's a rephrase, cut it. The pattern of three is a stylistic crutch; it's only worth using when each item adds genuinely new information. Two facts beat three padded ones.
- [ ] **The general rule.** A paragraph that contains a fact + a stylistic frame is almost always two sentences of frame around one sentence of fact. Cut to the fact. The reader is smart enough to know context matters when context is provided; they don't need to be told that it matters.
