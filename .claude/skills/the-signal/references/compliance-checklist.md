...
Also still run, as mechanical production steps (not editorial gates): `verify-orchestrator-model.sh` (Step Zero), `stitch-issue.sh` (holiday rewrite + banned-vocab scan), `check-release-dates.sh` (Phase 7.5 fact-date surface), and `check-image-diversity.sh` (Phase 7.7 — image diversity on **two** axes: the provenance axis it always had, plus, since 2026-07, the `min_distinct_shapes` floor on what the pictures actually *show*; see § Image specificity check).
...
- [ ] **Image specificity check (the "random Pope photo" rule) — HALF MECHANICAL since 2026-07.** A generic photo of a subject is almost never the right image. Before using ANY image, ask: does this picture show the SPECIFIC thing the section is about?

  **The hierarchy of preference is INFORMATION GAINED, not provenance (SPEC 2026-07-26 §3.13).** The `shows` value in brackets is the machine record; the enum is canonicalised in `references/image-source-types.json`.
  1. **A photograph or still of the actual thing happening** (`event_photo`, `gameplay`).
  2. **A diagram, map or chart carrying information the prose cannot** (`diagram`, `map`, `chart`) — this is a *promotion*: it used to rank below press-kit art, which is exactly backwards. A figure that explains something is worth more than a picture of the box it came in.
  3. **The artefact or the primary document itself** (`artefact`, `document`) — the object in the museum case, the store listing, the page of the record. *Note the deliberate asymmetry: of these two, only `artefact` satisfies the Long Read's information-figure requirement (§3.8 names `diagram`/`map`/`chart`/`artefact`). A store listing is a primary source; it is not a figure that explains anything.*
  4. **An in-engine or in-context still** (`in_engine`) — a render or cutscene with no HUD. Better than key art; not gameplay.
  5. **Key art, product shot, posed portrait** (`key_art`, `product_shot`, `portrait`) — **LAST RESORT, and never a lead figure.** Logo-composited marketing art tells the reader nothing the headline didn't. **`portrait` is in this rung on purpose, and the restriction costs nothing:** a *posed* portrait is `portrait`, but a photograph of a person actually *doing* the thing is `event_photo` and may lead freely. Even a profile or an obituary should lead on the subject in action — which is both available and better. All three values are in `shows.never_lead_shapes` in the lookup file; the prose and the machine list are the same rule written twice.

  If you can't find a specific image, sometimes the right answer is no image at all rather than a generic one. **Commons is not the problem and is no longer capped** — the old `wikimedia_max_pct` / `wikimedia_max_count` ceilings are retired (SPEC §3.14), because a ceiling with no matching floor reads as a target and Commons became the thing to fill up to. Commons is right when it holds the most *informative* image and wrong when it holds the most *convenient* one. What enforces that now is a floor on shapes, not a cap on a domain.

  **Worked example — the Pope (unchanged, still correct).** A profile portrait of the Pope (`portrait`, rank 5) is wrong for a story about his Africa trip — find a wire photo of him in Equatorial Guinea (`event_photo`, rank 1), or better, a map of the trip route (`map`, rank 2, because it carries information the prose doesn't).

  **Worked example — Halo (Issue #18, the failure that rewrote this rule).** The Pixel & Byte lead was Halo key art with the logo composited in (`key_art`, rank 5, used as a *lead*): it told the reader nothing the headline hadn't. The old wording actively prescribed it — it ranked "official art/press kit for the specific subject" at 2 and instructed "find official art for that game". The researcher obeyed the spec. **The right image is a real gameplay screenshot:** Steam's `appdetails` API (`https://store.steampowered.com/api/appdetails?appids=<id>`) returns a `screenshots[]` array of actual in-game shots for the game — verified 2026-07-26: app id `976730` (Halo: The Master Chief Collection) returns 15 of them — and the CDN they live on, `shared.fastly.steamstatic.com` / `shared.akamai.steamstatic.com`, is in the domain map. Take `path_full` for the full-resolution file. Same for the Long Read: the 2021 *Scientific Reports* paper's own CAD render of the gearing (CC BY) sat on `media.springernature.com`, also already whitelisted, while the issue used a 2007 hobbyist perspex model off Commons. **In both cases the source list was fine. The taxonomy was the bottleneck** — it typed images by domain and never asked what they showed.

  **Which half is mechanical (read the exit codes — do NOT hand-check these):**
  - `check-image-diversity.sh` (Phase 7.7), issue-wide: at least `min_distinct_shapes` (3) distinct `data-shows` values across the issue, and every `data-shows` value inside the canonical enum (a typo'd shape is invisible to every budget below, so it hard-fails).
  - `validate-issue.py` (gate 1), per band (SPEC §3.8): at most one `key_art` in Pixel & Byte and never as its lead plate; the Long Read carries ≥1 of `diagram`/`map`/`chart`/`artefact`; a Touchline figure captioned as a concluded result must be `event_photo`; a `src` that led the previous issue may not lead this one; plus the caption-vintage rule (§3.9 — if the capture year predates the band's dated claims, the visible caption must say the year).

  **Which half stays judgement (this checkbox).** Whether *this* picture is the most specific available picture of *this* thing. No script can tell a photograph of the right stadium from a photograph of the wrong one, or notice that the diagram it counted explains something no reader was asking about. The machine counts shapes; you judge fit.

  **Why the split is spelled out here.** When `visual-smoke-test.py` was deleted in v8.38 the ledger recorded "image intent → gate 1" — but gate 1 only checked HTTP 200. The specificity intent was dropped on the floor and nothing noticed until Issue #18 shipped key art as a lead. The mechanical half is now genuinely inside gate 1 + Phase 7.7, and the enum lives in exactly ONE place (`references/image-source-types.json`, mirrored for readers in `references/spec/data-contracts.md`). If a shape value appears in a script but not in that file, that is the drift starting again — fix the file, not the script.
...
- [ ] **Image-SHAPE diversity (MECHANIZED — read the exit code).** Satisfied by a PASS from `bash scripts/check-image-diversity.sh output.html`, which enforces `min_distinct_shapes` (≥3 distinct `data-shows` values issue-wide) and rejects any value outside the canonical enum. The eyeball backstop: `grep -oE 'data-shows="[a-z_]+"' output.html | sort | uniq -c` — if the list is one or two shapes long, or is all `key_art`/`product_shot`/`portrait`, the issue is illustrated with the safe shape everywhere and it will read as boring no matter how good the prose is. Diversity of *domains* does not fix this; only different *kinds of picture* do. See § Image specificity check for the hierarchy.
