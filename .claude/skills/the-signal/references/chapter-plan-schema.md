    ...
    "chapters": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "pieces": {
            "type": "array",
            "description": "For fixed-section weekly chapters with chapter_id in {world, pixel_byte, toolkit, touchline, screen_sound, session}. v8.34: 0-2 pieces — the considered piece as role=lead (synthesis / a named-layer roundup / an angle / a feature) plus an OPTIONAL role=companion (a companion requires a lead). The considered piece (a lead) is the mandatory backbone: a section must carry a lead OR a `yield_reason`. The `catch_up` roundup is OPTIONAL grounding, not a mandatory element — a section with only catch_up and no lead/yield_reason hard-fails (it must yield; recap is the daily's job). A bare lead with no catch_up is fine. If a companion is present, Lead.topic_family MUST differ from Companion.topic_family. Validator hard-fails: >1 lead, >1 companion, a companion without a lead, a companion sharing the lead's topic_family, a catch_up-only section (no lead, no yield_reason), or an empty section.",
            "minItems": 0,
            "maxItems": 2,
            "items": {
              "type": "object",
