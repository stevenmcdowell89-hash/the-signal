...
**What the cover leads on is a declared decision (v8.44, WP-2).** The markup is chrome; the *choice* is editorial and it is written down. Every weekly plan sets **`issue_meta.cover_leads_on`** — `"news"` or `"long_read"` — with **`issue_meta.lead_rationale`** (≥120 chars, naming what was considered and rejected).

- **`cover_leads_on: "long_read"` is a normal week, not an excuse.** A strong feature taking the cover on a thin news week is what a Sunday magazine does. The issue always has a Long Read; on a week when the news is a holding pattern and the anchor is genuinely good, the anchor is the better front page. Reach for it rather than promoting a story that hasn't moved. The stitcher reads the field: on `"long_read"` the tuner **station list leads with the Long Read**, so the choice is visible on the cover, not just in the plan.
- **The planner sees nine-plus weeks of cover history.** State `cover_lead_ledger[]` holds the last 12 covers, newest first, each `{ issue, date, topic_family, one_line, led_on }`. It exists because the previous input — `last_cover_lead` — showed **one** week, and one week of history cannot show a rut: UK politics led the cover in **6 of 9 issues** while every individual decision looked defensible on the day. Read the ledger before choosing, and if the same `topic_family` is about to lead a fourth time in five weeks, `validate-chapter-plan.py` requires **`issue_meta.lead_override_reason`** (≥80 chars) in the plan.
- **Not a suppression gate.** `check-topic-lock.py` is retired and stays retired. Nothing here makes a subject ineligible; the rut rule asks for a written justification, not a veto. If a rut recurs, strengthen the demotion below — do not add a check.

**Demotion principle** *(the cover's analogue of the Touchline's, below)*. When the cover lead changes hands — the rut rule bites, a feature takes the front, or a running story stops moving — **the demoted story falls to THE CLOSE. It does not vanish, and it does not linger on the cover at half strength.** The Close is already the "other important stuff still running" band, so a demoted lead lands where the reader looks for exactly that:

1. **A Threads recap** in `the_threads` — the standard "previously on / where it stands now" thread (mono `.ep` tag + `<h3>` + `.prev` + a `<p>` closing on `<b>Next:</b>`). This is where a demoted story keeps its continuity: it was the cover for weeks, so drop it cold and the reader loses the thread.
2. **A two-line policy ledger** beside it — the `demoted_lead_ledger` furniture object: a `.mx-ledger` carrying **`data-role="demoted-lead"`** (`data-mx-event="ledger"`), **two rows** of what moved, what did not, and what the story is now waiting on. Two lines, not two paragraphs: it is a ledger, not a rescued Lead. It renders in the **quieter** voice by design — vermilion belongs to what leads.

**Why the close and not a smaller cover slot.** The owner's judgement: *"the close is probably the best place for it — demoted pieces should fall to the close, it feels like a 'this is the other stuff happening that's important' type area."* The alternative shapes are worse. A demoted story kept near the front competes with the new lead and the cover reads as hedged. A demoted story cut outright is the failure the retired suppression gate produced — coverage disappearing because a counter tripped, rather than moving to where it belongs. Falling to the close is the third option: the story is still covered, honestly, at the weight it now earns. A story that stops producing developments at all leaves The Threads too (§ The Threads — a thread with no genuine development can be held) — but that is the story ending, not a demotion.
...
**When the Long Read is `vintage: "evergreen"`, The Letter must introduce it as a feature (v8.44, WP-2).** Give it **its own sentence**, and put `data-lr-framing="feature"` on the paragraph that does it — the markup gate checks the attribute is present, the wording stays yours. **Never fold a standing story into the paragraph listing what happened this week.** #18's Letter closed its news round-up with "*and a corroded lump of bronze pulled from a Greek shipwreck turned out, once the X-rays got into it, to have been a computer*" — a 1901 shipwreck and a 2021 paper, delivered in the same breath as Sunday's race and Tuesday's game release, and the reader read it as a discovery made this week. Name it as the issue's standing feature and let its span show; the span is the promise, not the caveat. (Full rule: § The Long Read in `editorial-spec.md`.)
...
- **Every row declares `source_band` (v8.44, WP-2)** — the `chapter_id` of a chapter present in this plan, or the literal `"state"` for the personal rows above (which have no originating band). A row sourced from a band must be a number that **moved inside the coverage window**. **Hard exclusion:** when the Long Read is `vintage: "evergreen"`, **no row may take `source_band` = the `long_read` chapter** — #18 filed `Antikythera Fragments · 82` between `Leaders in a Decade · 7` and Bank Rate, which reads as this week's news and undoes the standing-story framing in four words (§ The Long Read in `editorial-spec.md`).
...
**Two jobs, not one — results-of-record and the one story worth reading (v8.44, WP-2).** The section had a single job description and ≤600 words to do it in, so the narrative job won every week and coverage collapsed to whichever competition supplied a fresh, table-shaped result. That is a structural outcome, not a writing failure: five furniture objects and a considered piece in 600 words can only be about one thing. The band now has **two explicitly separate jobs**, and prose target rises to **500–800 words** so both fit. *This raises prose; it does not cut furniture.*

**Job 1 — the results ledger (comprehensive, calendar-driven, results-of-record).** Every tracked event that **concluded** since the previous `research_cut_at` (the plan's `issue_meta.window.from`), one line each. **Not fixtures** — the week ahead's dated fixtures moved to On the Radar as the separate `fixtures_lookahead` object, which must *not* carry `data-role="results-ledger"`. A ledger that mixed concluded results with pending kick-offs is how #17 reported a World Cup final as still to be played and then never reported the result. The rules:

- **Breadth is the point, and it is nearly free.** A ledger row costs about **twelve words** — a date, a name, a result. Adding a second and third sport costs a sentence and a half of prose, not a section. There is no editorial economy in which "no room" is the honest reason the ledger carried one competition.
- **Every row declares its sport.** The ledger renders as `.mx-ledger` carrying `data-mx-event="ledger"` and **`data-role="results-ledger"`** (furniture component `results_ledger`, renamed from `fixtures_ledger` by SPEC §3.11), with **`data-sport`** on each `.mx-ledger__row`.
- **`data-sport` carries the *sport*, never the competition, and never `multi_sport`.** The token vocabulary is sport-level and lowercase `snake_case` — `football` not `premier_league`, `motorsport` not `f1`; the canonical list lives in `references/spec/data-contracts.md` § Sport tokens (adding one is a one-line change there, not a decision made in prose). `multi_sport` is legal in exactly one place — **`sports_calendar[].sport`**, where it classifies an *event* (a games) that has no single sport — and is **forbidden** on `data-sport` and as an `interest_depth` key. A result belongs to a sport: rows derived from a multi-sport games carry `athletics`, `swimming`, `boxing` and so on. Collapsing a games into one token would let a ledger of `[motorsport, multi_sport]` satisfy "≥2 distinct" while telling the reader nothing about breadth — which is the defect, not the fix.
- **≥2 distinct sports whenever ≥2 concluded.** The `results_ledger_multi_sport` invariant: if two or more tracked sports had an event conclude inside the window, the ledger must carry **≥2 distinct `data-sport` values**. `validate-issue.py` fails a single-sport ledger on such a week.
- **Source it from the calendar, not from feed volume.** Read state `sports_calendar[]` — `{ event, sport, start, end, importance, reader_relevant, needs_verification }` — for what ended in-window. An event whose `needs_verification` is `true` gets its dates confirmed against a source before the planner relies on it. Feed volume is a measure of how much a sport publishes, not of what happened.
- **An event that concludes *after* the research cut is an open loop, not a gap.** Its result is not knowable this week and must not be guessed. It gets a pending mention naming the event, the date and the participants; it opens an entry in state `open_loops[]`; and next week's band **owes the result**, carrying `data-resolves-loop="<id>"` (§ Content Standards → the coverage window and open loops, in `editorial-spec.md`). This is how a Sunday final stops disappearing.
- **Standings render polymorphically.** The `.mx-scorecard` takes `data-table-kind` ∈ `league` · `medal` · `gc` · `leaderboard` · `championship` — a league table, a medal table, a general classification, a leaderboard, a championship standing. A section whose only table shape was "league" could only ever show league sports. The `.mx-dial` signature is **optional** and retargetable to whatever leads; it is not a reason for motorsport to lead.

**Job 2 — one lead story.** *One*, chosen by the priority hierarchy below, allowed to **yield** when nothing has earned it. The ledger having covered the results means the lead is free to be the story genuinely worth reading rather than the story that happened to have a table. A lead that is a play-by-play recap of an event the reader watched is not a lead (§ Article Structure).
...
> **The hierarchy above is sound and was simply not followed (v8.44, WP-2 — audit finding, do not rewrite it).** Read against Issue #18 it gives the right answer and gives it unambiguously. Round 11 of a 24-round motorsport season is a **normal domestic week, priority 6** — it is not a title decider, so it does not reach priority 2 — while a **UK-hosted multi-sport games** (Glasgow 2026, opening 23 July, inside the window) is squarely **priority 2**, a major non-football event that "can take the lead and push football into a secondary role". #18 led the section on the priority-6 item, gave one race **five furniture bands plus a mention in The Letter**, and never mentioned the priority-2 event at all. Golf — which *is* in the reader profile, with five daily feeds — got **zero** coverage in the issue whose window contained The Open's final round, and cricket has appeared **zero** times across all sixteen issues. None of that is a defect in the ranking; it is the ranking being bypassed. So the fix is **not** a new hierarchy. It is the results ledger above (breadth stops depending on the lead), the calendar-driven scan below (the section can *see* the priority-2 event), and stronger demotion language (immediately below). If the hierarchy is ever genuinely wrong, amend it deliberately — do not add a rule beside it.

**Demotion principle.** When a story from priorities 1–3 takes the lead, everything below it compresses cleanly: condensed results into the ledger, the table, a one-line note on anything genuinely significant (sacking, title decided, record). Don't try to give the demoted subject equal weight on those weeks — the section is finite and the lead has earned its space.

**Demotion runs the other way too, and this is the half that was missing (v8.44, WP-2).** The rule was written to protect a big story from being crowded out by routine football. The failure it did *not* cover is the mirror image: **a routine, weekly-recurring, table-shaped competition holding the lead against something higher in the hierarchy.** That demotion is not optional and not a matter of taste.

- **Priority 6 yields to priorities 1–3 without negotiation.** A normal round of any weekly series — a league matchday, a race in the middle of a season, a tour stop — has no claim on the lead in a week when a tournament, a major non-football event, a multi-sport games, or a structural story is live. Its results go in the ledger, which is what the ledger is for.
- **"It produced a fresh table" is not a priority.** Weekly recurrence makes a competition *easy to write*, not *important*. A sport that publishes a classification every seven days will always look like it has news; that is a property of its calendar, not of the week.
- **One competition does not take multiple furniture objects.** The Touchline's furniture (`results_ledger`, `standings_card`, `ticket`, `quote_objects`, the optional `dial_signature`) is there to distribute *events across the section*, not to give one event five renderings. Retarget the objects, or drop the ones that have nothing distinct to hold. Five bands on one race is the single clearest symptom of a bypassed hierarchy.
- **Depth is weighted by `interest_depth`, per sport.** Read state `interest_depth` — values `full` · `majors_only` · `results_only` · `off`. Seeded: **motorsport `results_only`**, football `full`, golf `majors_only`. `results_only` means exactly what it says: **the big results are in scope, in the ledger; session-by-session, qualifying-by-qualifying and paddock process are out.** *"F1 is at best a passing interest — nice to know big results, I don't need a minute by minute."* Motorsport is **not in the reader profile's interest list** at all (`references/spec/global.md` § The Reader names football and golf), which is why its depth is stated rather than inferred from how much its feeds produce.
- **An absent `interest_depth` key means *unset*, never `off`.** Only the sports the owner has explicitly weighted carry a key. Cricket, cycling, athletics, swimming, tennis, rugby and the rest have **no key** — they are covered **on news value**, like anything else in the magazine (§ Lens, not Filter: the profile narrows angle, never appetite). Reading an absent key as "off" would recreate the invisible-sport problem in a new place, which is the whole defect.

**The weekly scan is calendar-driven, not a list of competitions (v8.44, WP-2 — replaces the fixed scan list).** The previous rule told the researcher to scan "golf, F1, Olympics buildup, rugby, tennis" — and a fixed list can only find what is on it. Of those five, **only one returns a fresh, table-shaped result every week of its season**, so a rule written to *broaden* coverage reliably narrowed it to that one; and the list omitted cycling, cricket, athletics, swimming and multi-sport games entirely. Ask three questions instead, answered from state **`sports_calendar`** (and confirmed against a source — every seeded entry carries `needs_verification: true`):

1. **What concluded** between the previous `research_cut_at` and now? → the results ledger. Every sport, `importance` 1 first.
2. **What is running** now — a tournament mid-flight, a stage race, a games, a series in season? → the lead candidate, ranked by the priority hierarchy.
3. **What opens in the next week?** → On the Radar, and a pending mention plus an open loop for anything concluding after the research cut.

Then the domestic beat (Juventus + Serie A, Premier League, CL/EL in season) and structural/governance stories across all sports (priority 3). An event in the calendar with `reader_relevant: true` and `importance: 1` that gets **no mention at all** is a coverage failure regardless of how good the lead was. If a sport is missing from `sports_calendar`, add it once its dates are confirmed — a gap in the calendar is a gap in the magazine, and the calendar is maintained, not remembered.
...
- **Markup (band `touchline`):** a `.lead` (with a `.drop` opener) → the **results ledger** (`.mx-ledger[data-mx-event="ledger"][data-role="results-ledger"]`, one `.mx-ledger__row[data-sport]` per concluded event) and the standings `.mx-scorecard[data-table-kind]` → an `.items` list (mono `.freq` + `<h3>` + `<p>`) for the secondary stories. The `.scores` grid of `.score`(`.wide`) cards remains available for a marquee result the lead is about. Data first, then narrative — and the data is now *all* the week's sports, not the lead's sport.
...
- **Golf is in the reader profile** (`majors_only`) and its majors get proper coverage when in season — they can lead the section. A major concluding inside the window and getting no ledger row is a failure, not a judgement call: The Open's final round was in #18's window and the issue carries the word "golf" zero times.
- Football reads like editorial, not match reports — the reader already knows the scores

**Considered-piece backbone (v8.27, inverted v8.34; split v8.44, WP-2):** one considered piece (the most compelling sport story that passes the two-factor test — lead with the angle, never a play-by-play recap of a match the reader watched), and **the results ledger is not it** — the ledger is results-of-record and runs whether or not a considered piece does. The Catch-Up roundup is optional grounding, but **when it runs it must carry the football the reader actually wants** — transfer rumours and confirmations, squad/World-Cup announcements, what's coming up. The old failure was spending the whole section on "Arsenal won" and dropping the transfer catch-up entirely; the newer failure was spending it on one race. An optional Companion: when the Lead is football, a Companion must be a non-football sport (golf, cricket, cycling, athletics, rugby, tennis, motorsport, governance/structural); when the Lead is a Priority-2/3 non-football story, a Companion may be football.
...
- **The Threads is where a demoted cover lead lands (v8.44, WP-2).** This band is already the issue's "other important stuff still running" area, so it is where a story that lost the cover goes — a normal thread recap, plus the `demoted_lead_ledger` object alongside it: a `.mx-ledger` carrying **`data-role="demoted-lead"`** (`data-mx-event="ledger"`), **two rows** of what moved, what did not, and what the story is waiting on. Two lines, not two paragraphs — a demoted lead becomes a ledger, not a rescued Lead, and it renders in the quieter voice because vermilion belongs to what leads. Full doctrine and rationale: § Cover → **Demotion principle**.
...
> **The cover rut rule is not a revival of that cap (v8.44, WP-2).** State `cover_lead_ledger[]` and the rut rule (§ Cover; `editorial-spec.md` § What to Lead With) look superficially like the retired sliding window and are a different mechanism. The cap **removed a subject from eligibility** on a count. The rut rule removes nothing: it requires **`issue_meta.lead_override_reason`** — a written justification in the plan — before the same `topic_family` leads the cover a fourth time in five weeks, and the planner may always lead with it. `ongoing_stories` remains suppression-free and single-purpose (The Threads). Also note `lead_history` inside `ongoing_stories` is a **story-thread** record and is *not* interchangeable with `cover_lead_ledger`, which follows the **rendered covers**; do not reconcile one against the other.
