...
- **A result that concluded after the research cut is an open loop, not a gap.** It gets a pending mention this week and is *owed* next week, resolved with `data-resolves-loop` (§ Content Standards → open loops, in `editorial-spec.md`). This is how a Sunday final stops disappearing.
